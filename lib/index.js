...
module.exports = async function (fd, { autoClose = true } = {}) {
  ...
  const findIcon = async () => {
    const matchIcon = f => reg.icon.test(f);
    const { entry } = await findEntry(zip, matchIcon);
    if (!entry) {
      return null;
    }
    const file = await zip.openReadStreamAsync(entry);
  ...
  const [info, mobileprovision, icon] = await Promise.all([findInfo(), findProv(), findIcon()]);
  return {
    info,
    mobileprovision,
    icon
